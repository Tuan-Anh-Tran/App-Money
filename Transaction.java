...
public class Transaction {
    ...
    public boolean processTransaction() {
        ...
        for(TransactionInput i : inputs) {
            i.UTXO = TAT_Chain.UTXOs.get(i.transactionOutputId);
        }

        //Kiểm tra giao dịch hợp lệ
        if(getInputsValue() < TAT_Chain.minimumTransaction) {
            System.out.println("Dau vao cua giao dich qua be: " + getInputsValue());
            System.out.println("Vui long nhap so tien giao dich lon hon" + TAT_Chain.minimumTransaction);
            return false;
        }
        ...
        for(TransactionInput i : inputs) {
            ...
            TAT_Chain.UTXOs.remove(i.UTXO.id);
        }
